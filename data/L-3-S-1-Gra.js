(function() {
    ...
    const slides = [
        ...
        { 
            ...
            extra: 'an action is happening right now' 
        },
        ...
        { 
            ...
            content: `
                ...
                <p style="font-size:2.5vw; color:#666; margin-top:5vh; font-weight:bold;">Example: I <span style="color:#f1c40f;">am</span> speak<span style="color:#e74c3c;">ing</span> to you.</p>
            ` 
        },

        /* 4: Reveal List (Live Actions) */
        { 
            type: 'reveal-list', 
            title: 'Live Actions',
            items: [
        ...
        /* 5: Negative Transform (The requested 3 sentences) */
        { 
            type: 'big-list', 
            title: 'The Negative',
            desc: 'Just add "NOT" after am/is/are',
            items: [
                "I'm <span style='color:#e74c3c;'>NOT</span> playing",
                "She is <span style='color:#e74c3c;'>NOT</span> cooking",
                "You are <span style='color:#e74c3c;'>NOT</span> teaching"
            ]
        },

        /* 6: Question Transform (The requested 3 sentences) */
        { 
            type: 'big-list', 
            title: 'The Question',
            desc: 'Switch Am/Is/Are with the Subject',
            items: [
                "<span style='color:#f1c40f;'>AM</span> I late?",
                "<span style='color:#f1c40f;'>IS</span> he your brother?",
                "<span style='color:#f1c40f;'>ARE</span> they your friends?"
            ]
        },
    ...
    function updateSubSteps() {
        ...
        if (!s) return;
        if (s.type === 'reveal-list' || s.type === 'big-list') {
            const items = container.querySelectorAll('.list-item');
            items.forEach((item, i) => {
                item.style.opacity = (i <= subStep) ? '1' : '0.05';
                item.style.transform = (i <= subStep) ? 'scale(1)' : 'scale(0.9)';
            });
        }
    }
    ...
    function renderSlide(index) {
        ...
        wrapper.style.cssText = `width:95%; height:95%; display:flex; flex-direction:column; justify-content:center; align-items:center; animation: vetoSlideUp 0.4s ease; text-align:center;`;
        ...
        if (s.type === 'title') {
            ...
            wrapper.innerHTML = `
                ...
                <div style="position:relative; width:80%; height:1vh; background:#222; border-radius:10px; margin-bottom:10vh;">
                    ...
                    <div class="timeline-dot" style="position:absolute; left:50%; top:-1.5vh; width:3vh; height:3vh; border-radius:50%; background:#e74c3c; box-shadow:0 0 15px #e74c3c; transform:translateX(-50%); animation: pulse 1.5s infinite;"></div>
                </div>
                <div style="font-size:3.5vw; color:#f1c40f; font-weight:900; text-transform:uppercase; letter-spacing:2px;">${s.extra}</div>
            `;
        }
        ...
        else if (s.type === 'big-list') {
            wrapper.innerHTML = `
                <h2 style="font-size:3vw; color:#3498db; margin-bottom:2vh; font-weight:900;">${s.title}</h2>
                <p style="font-size:1.5vw; color:#555; margin-bottom:4vh; font-weight:bold;">${s.desc}</p>
                <div style="display:flex; flex-direction:column; gap:3vh; width:90%;">
                    ${s.items.map(item => `<div class="list-item" style="opacity:0.05; transition:0.4s; background:rgba(255,255,255,0.03); padding:3vh 2vw; border-radius:20px; font-size:4.5vw; font-weight:900; border:2px solid #222; color:#fff;">${item}</div>`).join('')}
                </div>`;
        }
        else if (s.type === 'spelling-grid') {
            wrapper.innerHTML = `
                <h2 style="font-size:3.5vw; color:#3498db; margin-bottom:4vh; font-weight:900;">SPELLING RULES</h2>
                <div style="display:grid; grid-template-columns: 1fr; gap:2vh; width:85%;">
                    ${s.rules.map(r => `
    ...
    function renderQuiz(wrapper, s) {
        ...
        wrapper.innerHTML = `
            <div style="text-align:left; background:#111; padding:4vh 4vw; border-radius:30px; border:2px solid #222; width:85%;">
                ...
                <div style="font-size:2.5vw; font-weight:900; margin-bottom:3vh; color:#fff;">${q.q}</div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1.5vw;">
                    ${q.opts.map((opt, i) => {
                        let isCorrect = showAns && i === q.ans;
                        return `<div style="border:${isCorrect ? '4px solid #2ecc71' : '2px solid #333'}; padding:2vh; border-radius:15px; font-size:1.8vw; font-weight:bold; color:${isCorrect ? '#2ecc71' : (showAns ? '#444' : '#fff')}; transition:0.3s;">${opt}</div>`;
                    }).join('')}
                </div>
            </div>`;
    }
    ...
    document.onkeydown = (e) => {
        ...
        if ([13, 32, 39].includes(e.keyCode)) {
            let max = 0;
            if (s.type === 'reveal-list' || s.type === 'big-list') max = s.items.length - 1;
            else if (s.type === 'timeline') max = 0;
            else if (s.type === 'quiz-session') max = (s.questions.length * 2) - 1;

            if (subStep < max) {
                subStep++;
                if (s.type === 'quiz-session') renderQuiz(container.querySelector('.slide-wrapper'), s);
                else updateSubSteps();
            } else if (currentSlide < slides.length - 1) {
                renderSlide(currentSlide + 1);
            } else if (typeof closeStage === 'function') closeStage();
    ...
    style.innerHTML = `
        ...
        @keyframes pulse { 0% { transform: translateX(-50%) scale(1); box-shadow:0 0 10px #e74c3c; } 50% { transform: translateX(-50%) scale(1.4); box-shadow:0 0 30px #e74c3c; } 100% { transform: translateX(-50%) scale(1); box-shadow:0 0 10px #e74c3c; } }
    `;
