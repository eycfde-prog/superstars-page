(function() {
    ...
    const rawWords = [
        "School", "Classroom", "Desk", "Chair", "Institute", "Whiteboard", "Chalk", "Marker", "Eraser", "Pen", "Pencil", "Sharpener", "Ruler", "Notebook", "Book", "Paper", "Backpack", "Pencil case", "Scissors", "Glue", "Tape", "Stapler", "Calculator", "Computer", "Laptop", "Tablet", "Homework", "Lesson", "Exam", "Grade", "Teacher", "Student", "Principal", "Library", "Laboratory", "Map", "Globe", "Dictionary", "Subject", "English", "Math", "Science", "History", "Geography", "Art", "Music", "Campus", "Break", "Schedule", "University"
    ];
    ...
    function showStartScreen() {
        container.innerHTML = `
            <div id="startExam" style="cursor:pointer; text-align:center; animation: fadeIn 0.8s ease;">
                <div style="font-size:3vw; color:#c5a059; letter-spacing:10px; font-weight:900; margin-bottom:10px;">SCHOOL V16</div>
                <div style="font-size:6vw; color:#fff; font-weight:900; text-transform:uppercase; line-height:1;">SPEED TEST</div>
                <div style="margin-top:40px; background:#c5a059; color:#000; padding:20px 60px; font-size:2vw; font-weight:900; border-radius:50px; display:inline-block; transition:0.3s;">START CLASS</div>
                <div style="margin-top:20px; color:rgba(255,255,255,0.3); font-size:1.2vw; letter-spacing:3px;">50 ACADEMIC WORDS • 3s AUTO</div>
            </div>
            <style>
    ...
    function renderWord() {
        ...
        // تحجيم الخط الذكي للأدوات التعليمية (Academic Optimized)
        if (word.length <= 5) { fontSize = '22vw'; } 
        else if (word.length <= 9) { fontSize = '16vw'; } 
        else if (word.length <= 13) { fontSize = '11vw'; } 
        else { fontSize = '9vw'; } // للكلمات الطويلة مثل Laboratory
        ...
        container.innerHTML = `
            ...
            <div style="text-align:center; width:95%;">
                ...
                <div id="vocabWord" style="
                    ...
                    letter-spacing:-1px; 
                    animation: schoolIn 0.3s cubic-bezier(0.23, 1, 0.32, 1);
                    white-space: nowrap;
            ...
            <style>
                @keyframes schoolIn { 0% { opacity:0; transform: translateY(-30px); } 100% { opacity:1; transform: translateY(0); } }
    ...
    function completeExam() {
        ...
        container.innerHTML = `
            <div style="text-align:center; animation: fadeIn 1s ease;">
                ...
                <div style="font-size:5vw; color:#c5a059; font-weight:900; letter-spacing:5px;">GRADUATED!</div>
                <div style="font-size:2vw; color:#fff; opacity:0.6; margin-top:10px;">School Vocabulary Speed Test Completed</div>
            </div>
        `;
