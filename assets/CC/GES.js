/**
 * GE.js - محرك تصحيح نشاط Golden Ear
 * تصميم: جي (Ge) لمستر عز
 */

function gradeMission(data) {
    const studentAnswers = data.answers; // مصفوفة الإجابات المختارة (5 إجابات)
    const taskNum = data.taskNum; // رقم المهمة لجلب نموذج الإجابة الخاص بها
    
    // نموذج الإجابات الصحيحة لجميع المهام (تم استخراجها من كود الصفحة)
    const geKeys = {
        "1": [
    ...
    const correctKeys = geKeys[taskNum] || [];
    let score = 0;

    // مقارنة إجابات الطالب بالنموذج
    studentAnswers.forEach((answer, index) => {
        if (answer === correctKeys[index]) {
            score += 1; // درجة لكل سؤال
        }
    });

    // تطبيق مبدأ الحد الأدنى (1 من 5)
    let finalScore = Math.max(1, score);
