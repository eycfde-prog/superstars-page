(function() {
    // 1. استهداف الحاوية الموحدة في الـ IFrame
    ...
    function render() {
        ...
        wrapper.style.cssText = `width:95%; text-align:center; animation: fadeIn 0.5s ease;`;

        // تم اختصار منطق الـ Render ليكون متوافقاً مع حجم الشاشة
        if (s.type === 'big-title') {
            ...
            wrapper.innerHTML = `<div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; max-width:900px; margin:auto;">
                ${s.items.map((item, i) => `<div style="font-size:2rem; background:#1e1e1e; padding:15px; border-radius:10px; display:flex; justify-content:space-between;">
    ...
    // 2. التحكم المطور بالكيبورد لدعم الـ Done Overlay
    document.onkeydown = (e) => {
        ...
        if (e.keyCode === 39 || e.keyCode === 13 || e.keyCode === 32) { // Next
            if (s.type === 'reveal-list' && subStep < s.items.length - 1) subStep++;
                ...
                // 3. استدعاء شاشة النهاية عند انتهاء كل شيء
